...
@foreach($salary as $key=>$item)
    @if($item->employee) <!-- Employee இருக்கான்னு check பண்ணுங்க -->
    <tr>
        <td>{{ $key+1 }}</td>
        <td>
        ...
        <td>
            ...
            <a href="{{route('delete.advance.salary',$item->id)}}" class="btn btn-danger rounded-pill waves-effect waves-light" id="delete">
                Delete
            </a>
        </td>
    </tr>
