...
<h3>Products</h3>
  ...
  <table width="100%">
    ...
    <tbody>
     ...
     @foreach($orderItem as $item)
      <tr class="font">
        <td align="center">
            @if($item->product->product_image_base64)
                <img src="{{$item->product->product_image_base64}}" height="50px;" width="50px;" alt="">
            @else
                <span>No Image</span>
            @endif
        </td>
  ...
  <table width="100%" style=" padding:0 10px 0 10px;">
    <tr>
        <td align="right" >
            <h2><span style="color: green;">Subtotal:</span>Rs.{{$order->total}}  </h2>
            <h2><span style="color: green;">Total:</span> Rs.{{$order->total}}  </h2>
